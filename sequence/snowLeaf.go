...
func (S *SnowflakeSeqGenerator) GenerateId(entity string, ruleName string) string {
	...
	if S.timestamp > now { // Clock callback
		fmt.Printf("Clock moved backwards. Refusing to generate ID, last timestamp is %d, now is %d", S.timestamp, now)
		return ""
	}
	...
	if tmp > timestampMaxValue {
		fmt.Printf("epoch should between 0 and %d", timestampMaxValue-1)
		return ""
	}
	S.timestamp = now
